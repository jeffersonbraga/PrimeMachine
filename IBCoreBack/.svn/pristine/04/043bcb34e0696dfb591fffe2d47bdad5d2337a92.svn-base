		...
		try {
			...
			if(profileToPost.getType() == Profile.TWITTER) {
				...
				if(wasSent) {
					...
					if(postDTO.getAppendType().equals(Post.APPEND_RESUME)) {
						mapPostsSent.put(false, OpSocialBackApplication.messageByLocaleService.getMessage("post.sent.notsuport", messageArgs));
					} else {
