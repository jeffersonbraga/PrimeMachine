				...
				try {
					...
					if(!profileMap.containsKey(profile.getNetworkId()) && new ReportsValidation().canGenerateReport(profile)) {
						...
						try {
							...
							if(reportGenerateRemote.hasEntity(profile.getNetworkId(), Profile.INSTAGRAM)) {
								
								if(reportGenerate.getState().equals(ReportFacebook.FINALIZED)) {
									...
									reportInstagram.refreshPostsInsights();
									...
									for(Long reportDay : reportDaysMain) {
										new ReportInstagram(profile, true).retrieveAllInsightsForDay(
												UtilFunctions.getDateForInstagramReport(LocalDateTime.ofEpochSecond(reportDay, 0, ZoneOffset.UTC)));
									}
								}
