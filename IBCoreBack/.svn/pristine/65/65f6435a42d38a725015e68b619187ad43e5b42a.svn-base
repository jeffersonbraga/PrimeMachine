...
/*
		...
		*/
        ...
        try {
			...
			//fields.add(new BasicNameValuePair("url_retorno","https://app.woopsocial.com/api/ipag_return"));
			fields.add(new BasicNameValuePair("url_retorno","https://app.woopsocial.com/api/ipag_return/profile_id/" + user.getAccount().getIdAccount()));
			...
			String date_start = ("01/" + month + "/" + cStart.get(Calendar.YEAR));
			fields.add(new BasicNameValuePair("inicio", date_start	));
			//fields.add(new BasicNameValuePair("ciclos",	valorAtt.toString()));	      

			fields.add(new BasicNameValuePair("valor",	valorAtt.toString()));

			canTryPayment = this.validateCanTryPayment(ipagDTO);
			...
			if (canTryPayment) {
				...
				cTransaction.setDate_start(date_start);
				...
				if (entity != null) {
		            ...
		            try {
		                 ...
		                 * 
		                 */
		                if (result != null && result.indexOf("8") > -1) {
							try {
								...
								new EmailSender().sendCheersEmail(acc);
		                ...
		                } else if (result != null && result.indexOf("7") > -1) {
		        			...
		        			String m = ipagDTO.getMes_cartao();
		        			if (new Integer(ipagDTO.getMes_cartao()) < 10) {
		        				m = "0" + ipagDTO.getMes_cartao();
		        			}

		        			cTransaction.setMes(m);
		        			cTransaction.setAno(ipagDTO.getAno_cartao());
		        			cTransaction.setLast4(ipagDTO.getNum_cartao());
		        			...
		        			if (result != null) {
		        				...
		        				result = "0";
		        				cTransaction = new CardTransaction();
		            			cTransaction.setNumpedido(null);
		            			cTransaction.setStatuspagamento("0");
		            			if (writer != null) {
		            				cTransaction.setMensagemtransacao(writer.toString());
		            			}
		            			cTransaction.setOperadoramensagem("Woop error message");
		            			cTransaction.setMetodo(ipagDTO.getMetodo());
            			...
            			cTransaction.setDate_event(new Date());
    			...
    			cTransaction.setNumpedido(null);
    			cTransaction.setStatuspagamento("0");
    			cTransaction.setMensagemtransacao("Blocked too many attempt");
    			cTransaction.setOperadoramensagem("Woop error message");
    			cTransaction.setMetodo(ipagDTO.getMetodo());
	...
	private boolean validateCanTryPayment(IpagDTO ipagDTO) {

		User user = SecurityUtils.getCurrentUser();
        MaintenanceCardTransactionRemote remoteCardTransaction = (MaintenanceCardTransactionRemote) RecoverMaintenance.recoverMaintenance("CardTransaction");

		String month = ipagDTO.getMes_cartao();
		if (new Integer(ipagDTO.getMes_cartao()) < 10) {
			month = "0" + ipagDTO.getMes_cartao();
		}

        List<CardTransaction> lstAttempt = remoteCardTransaction.getByAccount(
        			user.getAccount().getIdAccount()
        			,ipagDTO.getNum_cartao().substring(ipagDTO.getNum_cartao().length() - 4)
        			,month
        			,ipagDTO.getAno_cartao());

		if ((lstAttempt != null) && (lstAttempt.size() > 3)) {
			return false;
		} else {
			return true;
		} 
	...
	public ResponseEntity<Object> makeIpagTransaction(@Valid @RequestBody IpagDTO ipagDTO) {
        ...
        if (result.equals("5")) {
        	...
        	return new ResponseEntity<>("Check the form data", HttpStatus.INTERNAL_SERVER_ERROR);
        } else if (result.equals("-1")) {

        	return new ResponseEntity<>("Too many attempts with the same card. It will be unlock tomorrow. You can try with another.", HttpStatus.INTERNAL_SERVER_ERROR);
        } else {
