...
public class Teste {
	...
	public static void main(String[] args) {
		...
		List<?> lista = (List<?>) dao.getSession().createCriteria(Natureza_ocorrencia.class).list();
		CrudGenericBO crud = new CrudGenericBO();
		for (Natureza_ocorrencia item : (List<Natureza_ocorrencia>) lista) {

			for (int i = 0; i < 50; i++) {

				item.setCd_natureza_ocorrencia(null);
				try {
					crud.salvar(item);
				} catch (Exception e) {
